...
namespace RobotIM {
    public class vmTrg_IM {
        ...
        public PlotModel GetNewModel(string title = "", string xname = "", string yname = "") {
            ...
            colorAx = new LinearColorAxis { Position = AxisPosition.Right, Palette = OxyPalettes.Rainbow(500), HighColor = OxyColors.Gray, LowColor = OxyColors.Black };
            magAx = new MagnitudeAxis { Minimum = 0, Maximum = 25, MajorStep = 5, MinorStep = 1 };
            model.Axes.Add(new AngleAxis { StartAngle = 0, EndAngle = 180, Minimum = 0, Maximum = 180, MajorStep = 30, MinorStep = 15 });
            model.Axes.Add(magAx);
            
            model.Axes.Add(colorAx);
        ...
        public void Print(List<Experim1Info> lst) {
            var fis = lst.Select(inf => inf.TrgTetta0).Distinct().OrderBy(d => d).ToList();
            var rs = lst.Select(inf => inf.TrgR0).Distinct().OrderBy(d => d).ToList();
            var data = new double[fis.Count, rs.Count];
            foreach (var inf in lst) {
                data[inf.I_ind, inf.J_ind] = inf.Prob;
            }
            Pm.Series.Clear();
            colorAx.Maximum = 1d;
            colorAx.Minimum = 0d;
            magAx.Maximum = rs.Max();
            Pm.Series.Add(new PolarHeatMapSeries {
                Data = data,
                Angle0 = fis.Min(),
                Angle1 = fis.Max(),
                Magnitude0 = rs.Min(),
